...
const RaraNadifa = ({ videoId }) => {
    ...
    const getCharId = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8080/getvideos/3cfc85b3-1f02-4679-9c6e-e593aa756570`
        );
  ...
  const handleLikeClick = async () => {
    ...
    if (liked[videoId]) {
      try {
        const response = await fetch(`http://localhost:8080/videos/declike/3cfc85b3-1f02-4679-9c6e-e593aa756570`, {
          method: 'PUT',
        });
        if (!response.ok) {
      ...
      if (dislike && dislike[videoId]) {
        ...
        try {
          const response = await fetch(`http://localhost:8080/videos/incdislike/3cfc85b3-1f02-4679-9c6e-e593aa756570`, {
            method: 'PUT',
          });
          if (!response.ok) {
      ...
      try {
        const response = await fetch(`http://localhost:8080/videos/inclike/3cfc85b3-1f02-4679-9c6e-e593aa756570`, {
          method: 'PUT',
        });
        if (!response.ok) {
          throw new Error('Failed to like video');
  ...
  const handleDislikeClick = async () => {
    ...
    if (disliked[videoId]) {
      try {
        const response = await fetch(`http://localhost:8080/videos/decdislike/3cfc85b3-1f02-4679-9c6e-e593aa756570`, {
          method: 'PUT',
        });
        if (!response.ok) {
            ...
            <div className="mx-auto max-w-full sm:max-w-screen-lg py-6 sm:px-6 lg:px-8">
              <div className="relative h-0" style={{ paddingBottom: "56.25%" }}>
                <iframe
                  ...
                  src="https://vidhidepre.com/embed/lpno00dlro9y"
