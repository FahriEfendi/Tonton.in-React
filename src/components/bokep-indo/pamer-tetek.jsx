...
const RaraNadifa = ({ videoId }) => {
    ...
    const getCharId = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8080/getvideos/b244141a-dab0-4e9f-a540-5e32ecc82238`
        );
  ...
  const handleLikeClick = async () => {
    ...
    if (liked[videoId]) {
      try {
        const response = await fetch(`http://localhost:8080/videos/declike/b244141a-dab0-4e9f-a540-5e32ecc82238`, {
          method: 'PUT',
        });
        if (!response.ok) {
      ...
      if (dislike && dislike[videoId]) {
        ...
        try {
          const response = await fetch(`http://localhost:8080/videos/incdislike/b244141a-dab0-4e9f-a540-5e32ecc82238`, {
            method: 'PUT',
          });
          if (!response.ok) {
      ...
      try {
        const response = await fetch(`http://localhost:8080/videos/inclike/b244141a-dab0-4e9f-a540-5e32ecc82238`, {
          method: 'PUT',
        });
        if (!response.ok) {
          throw new Error('Failed to like video');
  ...
  const handleDislikeClick = async () => {
    ...
    if (disliked[videoId]) {
      try {
        const response = await fetch(`http://localhost:8080/videos/decdislike/b244141a-dab0-4e9f-a540-5e32ecc82238`, {
          method: 'PUT',
        });
        if (!response.ok) {
            ...
            <div className="mx-auto max-w-full sm:max-w-screen-lg py-6 sm:px-6 lg:px-8">
              <div className="relative h-0" style={{ paddingBottom: "56.25%" }}>
                <iframe
                  ...
                  src="https://d0000d.com/e/nuulc2u5h1m6"
