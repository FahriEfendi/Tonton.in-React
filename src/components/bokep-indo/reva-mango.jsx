...
const RaraNadifa = ({ videoId }) => {
    ...
    const getCharId = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8080/getvideos/daf97cfc-bfb0-4c9a-8ccc-21d7a40f9f7d`
        );
  ...
  const handleLikeClick = async () => {
    ...
    if (liked[videoId]) {
      try {
        const response = await fetch(`http://localhost:8080/videos/declike/daf97cfc-bfb0-4c9a-8ccc-21d7a40f9f7d`, {
          method: 'PUT',
        });
        if (!response.ok) {
      ...
      if (dislike && dislike[videoId]) {
        ...
        try {
          const response = await fetch(`http://localhost:8080/videos/incdislike/daf97cfc-bfb0-4c9a-8ccc-21d7a40f9f7d`, {
            method: 'PUT',
          });
          if (!response.ok) {
      ...
      try {
        const response = await fetch(`http://localhost:8080/videos/inclike/daf97cfc-bfb0-4c9a-8ccc-21d7a40f9f7d`, {
          method: 'PUT',
        });
        if (!response.ok) {
          throw new Error('Failed to like video');
  ...
  const handleDislikeClick = async () => {
    ...
    if (disliked[videoId]) {
      try {
        const response = await fetch(`http://localhost:8080/videos/decdislike/daf97cfc-bfb0-4c9a-8ccc-21d7a40f9f7d`, {
          method: 'PUT',
        });
        if (!response.ok) {
            ...
            <div className="mx-auto max-w-full sm:max-w-screen-lg py-6 sm:px-6 lg:px-8">
              <div className="relative h-0" style={{ paddingBottom: "56.25%" }}>
                <iframe
                  ...
                  src="https://d0000d.com/e/z2xal76b8jrv"
